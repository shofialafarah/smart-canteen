<x-app-layout>
    <div class="min-h-screen bg-[#121212] text-white py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-8">
                <a href="{{ route('seller.dashboard') }}" class="text-orange-500 hover:text-orange-400 text-sm font-bold flex items-center mb-4 transition font-body">
                ...
                <form action="{{ route('menu.update', $menu->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    @method('PUT') <div>
                        ...
                        <input type="text" name="nama_menu" value="{{ $menu->nama_menu }}" 
                            class="w-full bg-[#121212] border-white/10 rounded-2xl px-6 py-4 text-white font-body focus:border-orange-500 transition" required />
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label class="font-display text-orange-500 text-xs uppercase tracking-widest mb-2 block font-bold">Stok Tersedia</label>
                    ...
                    <div>
                        <label class="font-display text-orange-500 text-xs uppercase tracking-widest mb-2 block font-bold">Foto Produk (Kosongkan jika tidak diganti)</label>
                        <div class="flex items-center gap-4 mb-4">
                            <img src="{{ asset('storage/' . $menu->foto_menu) }}" class="w-20 h-20 rounded-xl object-cover border border-white/10">
                            <p class="text-xs text-gray-500">Foto saat ini</p>
                        </div>
                        <input type="file" name="foto_menu" class="w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-orange-600 file:text-white hover:file:bg-orange-500 cursor-pointer" />
                    </div>
